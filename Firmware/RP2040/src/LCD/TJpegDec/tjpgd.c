...
/*
 * 2017-12-17: Mods by Terje Io
 *             Updated to use portable integer types (stdint.h)
 *             Some code optimizations: for loops -> while loops, multiplications and divisions to shift
 *             Added byte reversed RGB888 -> RGB565 conversion option
 */
...
#define BYTECLIP(v) Clip8[(uint16_t)(v) & 0x3FF] // use uint_fast16_t?
...
inline uint8_t BYTECLIP (int_fast16_t val)
{
...
static
JRESULT create_qt_tbl (     /* 0:OK, !0:Failed */
    JDEC* jd,               /* Pointer to the decompressor object */
...
static
JRESULT create_huffman_tbl (    /* 0:OK, !0:Failed */
    JDEC* jd,                   /* Pointer to the decompressor object */
    const uint8_t* data,        /* Pointer to the packed huffman tables */
    uint_fast16_t ndata         /* Size of input data */
)
{
    uint_fast16_t d, i, j, b, hc, np, cls, num;
    uint8_t *pb, *pd;
    uint16_t *ph;
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        i = np;
        while (i--) {           /* Load decoded data corresponds to each code ward */
            d = *data++;
...
static
int_fast16_t bitext (            /* >=0: extracted data, <0: error code */
    JDEC* jd,           /* Pointer to the decompressor object */
    uint_fast16_t nbit   /* Number of bits to extract (1 to 11) */
)
{
    uint8_t msk, s, *dp;
    uint_fast16_t dc, v, f;
    ...
    do {
        if (!msk) {             /* Next byte? */
            if (!dc) {          /* No input data is available, re-fill input buffer */
                ...
                if (!dc) return 0 - (int_fast16_t)JDR_INP;   /* Err: read error or wrong stream termination */
            } else {
                dp++;           /* Next data ptr */
            }
            ...
            if (f) {            /* In flag sequence? */
                ...
                if (*dp != 0) return 0 - (int_fast16_t)JDR_FMT1; /* Err: unexpected flag is detected (may be collapted data) */
...
static
int_fast16_t huffext (               /* >=0: decoded data, <0: error code */
    JDEC* jd,               /* Pointer to the decompressor object */
...
)
{
    ...
    do {
        ...
        for (nd = *hbits++; nd; nd--) { /* Search the code word in this bit length */
            if (v == *hcode++) {        /* Matched? */
                jd->dmsk = msk; jd->dctr = dc; jd->dptr = dp;
                return (int_fast16_t)*hdata;         /* Return the decoded data */
            }
            hdata++;
        }
        bl--;
...
)
{
    ...
    uint_fast16_t blk, nby, nbc, i, z, id, cmp;
    int_fast16_t b, d, e;
    uint8_t *bp;
    const uint8_t *hb, *hd;
    const uint16_t *hc;
    ...
    for (blk = 0; blk < nby + nbc; blk++) {
        ...
        if (JD_USE_SCALE && jd->scale == 3)
            *bp = (*tmp >> 8) + 128;   /*!! If scale ratio is 1/8, IDCT can be ommited and only DC element is used */
        else
...
JRESULT mcu_output (
    JDEC* jd,   /* Pointer to the decompressor object */
    bool (*outfunc)(JDEC*, void*, JRECT*),  /* RGB output function */
    ...
    if (!JD_USE_SCALE || jd->scale != 3) {  /* Not for 1/8 scaling */
        ...
        for (iy = 0; iy < my; iy++) {
            ...
            for (ix = 0; ix < mx; ix++) {
                ...
                *rgb24++ = /* R */ BYTECLIP(yy + (((int_fast16_t)(1.402 * CVACC) * cr) >> CVACCDIV));
                *rgb24++ = /* G */ BYTECLIP(yy - (((int_fast16_t)(0.344 * CVACC) * cb + (int_fast16_t)(0.714 * CVACC) * cr) >> CVACCDIV));
                *rgb24++ = /* B */ BYTECLIP(yy + (((int_fast16_t)(1.772 * CVACC) * cb) >> CVACCDIV));
            }
        }

        /* Descale the MCU rectangular if needed */
        if (JD_USE_SCALE && jd->scale) {
            uint_fast16_t x, y, r, g, b, s, w, a;
            uint8_t *op;

            /* Get averaged RGB value of each square correcponds to a pixel */
            s = jd->scale << 1;  /* Bumber of shifts for averaging */
            ...
            for (iy = 0; iy < my; iy += w) {
                for (ix = 0; ix < mx; ix += w) {
                    ...
                    r = g = b = 0; y = w;
                    while (y--) {   /* Accumulate RGB value in the square */
                        x = w;
                        while (x--) {
                            r += *rgb24++;
                            g += *rgb24++;
...
#elif JD_FORMAT == 2
    /* Convert RGB888 to byte reversed RGB565 if needed */
    uint8_t *s = (uint8_t*)jd->workbuf, *d = s;
    uint_fast16_t w, n = rx * ry;
    do {
        w = (*s++ & 0xF8) << 8;     /* RRRRR----------- */
        w |= (*s++ & 0xFC) << 3;    /* -----GGGGGG----- */
        w |= *s++ >> 3;             /* -----------BBBBB */
...
#define LDB_UINT16(ptr)     (uint16_t)(((uint16_t)*((uint8_t*)(ptr))<<8)|(uint16_t)*(uint8_t*)((ptr)+1))
...
)
{
    uint8_t *seg, b;
    uint32_t ofs;
    uint_fast16_t n, i, j, len, marker;
    JRESULT rc;
    ...
    uint16_t xx = LDB_UINT16(seg);
    if (xx != 0xFFD8) return JDR_FMT1;  /* Err: SOI is not detected */
    ofs = 2;

    for (;;) {
        /* Get a JPEG marker */
        if (jd->infunc(jd, seg, 4) != 4) return JDR_INP;
        marker = LDB_UINT16(seg);       /* Marker */
        len = LDB_UINT16(seg + 2);  /* Length field */
        ...
        case 0xC0:  /* SOF0 (baseline JPEG) */
            ...
            jd->width = LDB_UINT16(seg+3);      /* Image width in unit of pixel */
            jd->height = LDB_UINT16(seg+1);     /* Image height in unit of pixel */
        ...
        case 0xC4:  /* DHT */
            ...
            if ((rc = create_huffman_tbl(jd, seg, len))) return rc;
            break;
        ...
        case 0xDB:  /* DQT */
            ...
            if ((rc = create_qt_tbl(jd, seg, len))) return rc;
            break;
...
{
    uint_fast16_t x, y, mx, my, rst, rsc;
    JRESULT rc;
